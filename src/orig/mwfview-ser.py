...
# license: GPL
#
# edition with uart serial decoder analysis :) 
#
...
class AnalysisByteMarks:
    """
    attempt to analyze data, identify each start bit,
    and place an axvline there 
    """
    def __init__(self, ax, xt, yt):
        print "Analysin' byte marks.. "
        self.ax = ax
        self.xt = xt
        self.yt = yt
        self.marks=[]
        ...
        while index < len(xt):            
            nowval=yt[index]
            #~ intime = xt[index] >= 0.005134 and xt[index] <= 0.005144
            #~ if intime: print index, xt[index], prevval, nowval
                
            if not started: # careful, must have "<= threshold" - else can skip for exaclt val of threshold (2.5)! 
                if nowval <= threshold and prevval > threshold:
                    started = 1
                    #~ if intime: print "!!! starting.."
                else:
                    prevval = nowval
                    index += 1 
            if started:
                if nowval > threshold:
                    started = 0
                    #~ if intime: print "started was fake"
                else:
                    #~ if intime: print "found.append, skip9ind"
                    self.found.append(index)
                    index += skip9ind # skip 9 periods so we're in stop bit, and start looking for start again
                    started = 0
                prevval = nowval
        print "found start bit transitions: ", len(self.found)
        self.getStats()
    ...
    def shouldIRender(self):
        xmin_datac, xmax_datac = self.ax.get_xlim()
        xrange = xmax_datac - xmin_datac
        # see how many bytes(startbits) are present in this range
        xminind = int(xmin_datac / dt)
        xmaxind = int(xmax_datac / dt)
        self.torender=[]
        # self.found should be ordered. 
        i = 0
        while i < len(self.found):
            tind = self.found[i]
            if tind >= xminind and tind <= xmaxind:
                self.torender.append( (tind, i) )
            if tind > xmaxind:  # since its ordered, no point in looking beyound xmax, 
                break           # , so we can exit the 'while' loop
            i+=1 
        #~ print "Range indices: ", xminind, xmaxind
        #~ print "Found bytes: ", len(self.torender)
        #~ print "      values: ", self.torender
        #~ indx = pylab.searchsorted(self.xt, [x])[0]
        return len(self.torender)
    ...
    def redraw(self):
        numbiv = self.shouldIRender() #number of bytes in view
        if numbiv <= 0:
            #~ print "returning", numbiv
            return            
        txlim = self.ax.get_xlim()
        #~ print "shouldIRender ", numbiv
        numb_ststmark = 30 # draw start stop marks for up to 30 bytes in view
        numb_anotord = 15 # draw ordinal annotation for up to 15 bytes in view
        numb_bitletr = 7 # draw ordinal annotation for up to 7 bytes in view        
        # clear marks first if not clear already 
        if len(self.marks) > 0: self.clearMarks()
        # if not rendering, return 
        if numbiv > numb_ststmark: 
            #~ print "returning, large", numbiv
            return
        ...
        #~ strbyte = ""
        byteval = 0 
        # we have all indexes in torender, just draw 
        index = i = 0 
        while i < len(self.torender):
            index = self.torender[i][0]
            bytordnum = self.torender[i][1]
            nowval=self.yt[index]
            
            # here we know we are below numb_ststmark in view, so draw start stop without conditions
            avl = self.ax.axvspan(self.xt[index], self.xt[index]+bitperiod, color='#00ff00')
            avlend = self.ax.axvspan(self.xt[index+skip9ind], self.xt[index+skip9ind]+bitperiod, color='#aa0000', alpha=0.6)
            self.marks.append(avl)                
            self.marks.append(avlend)
            
            if numbiv <= numb_anotord:
                # we should decode, to print annotations
                # extract/decode data of this byte - LSB is first
                b0 = 1 if self.yt[index+skip15ind] > threshold else 0
                b1 = 1 if self.yt[index+skip25ind] > threshold else 0
                b2 = 1 if self.yt[index+skip35ind] > threshold else 0
                b3 = 1 if self.yt[index+skip45ind] > threshold else 0
                b4 = 1 if self.yt[index+skip55ind] > threshold else 0
                b5 = 1 if self.yt[index+skip65ind] > threshold else 0
                b6 = 1 if self.yt[index+skip75ind] > threshold else 0
                b7 = 1 if self.yt[index+skip85ind] > threshold else 0
                byteval = 0 
                byteval += b0 << 0 
                byteval += b1 << 1 
                byteval += b2 << 2 
                ...
                byteval += b6 << 6 
                byteval += b7 << 7                         
                #~ strbyte = "%d %d %d %d %d %d %d %d" % (b7, b6, b5, b4, b3, b2, b1, b0)
                
                # add annotation on bottom - byte order number; textcoords='offset points', xycoords='data', also "%d: %s %02X"
                antorder = self.ax.annotate("%d: %s %d" % (bytordnum+1, repr(chr(byteval)), byteval ), xy=(self.xt[index], nowval),  
                    xytext=(self.xt[index], -0.8), 
                    bbox=dict(boxstyle="round", fc="0.8"), )
                antorder.set_font_properties(":size=8")
                antorder.set_fontname("Arial")
                self.marks.append(antorder)
                
                if numbiv <= numb_bitletr:
                    self.marks.append(self.ax.annotate("%d" % (b0), xy=(self.xt[index+skip15ind], self.yt[index+skip15ind]), xytext=(self.xt[index+skip15ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b1), xy=(self.xt[index+skip25ind], self.yt[index+skip25ind]), xytext=(self.xt[index+skip25ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b2), xy=(self.xt[index+skip35ind], self.yt[index+skip35ind]), xytext=(self.xt[index+skip35ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b3), xy=(self.xt[index+skip45ind], self.yt[index+skip45ind]), xytext=(self.xt[index+skip45ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b4), xy=(self.xt[index+skip55ind], self.yt[index+skip55ind]), xytext=(self.xt[index+skip55ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b5), xy=(self.xt[index+skip65ind], self.yt[index+skip65ind]), xytext=(self.xt[index+skip65ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b6), xy=(self.xt[index+skip75ind], self.yt[index+skip75ind]), xytext=(self.xt[index+skip75ind], threshold), zorder=-1))
                    self.marks.append(self.ax.annotate("%d" % (b7), xy=(self.xt[index+skip85ind], self.yt[index+skip85ind]), xytext=(self.xt[index+skip85ind], threshold), zorder=-1))
                # end if numbiv <= numb_bitletr
            i += 1 # increase while counter
        # end redraw while loop 
        # apparently it kills the axis range, re-set it! 
...
print "Loading chan1..."
chan1 = np.loadtxt("full-6633-ch1.dat")
print "Loading chan2..."
chan2 = np.loadtxt("full-6633-ch2.dat")
...
# ##### 'global' consts, for AnalysisByteMarks:
threshold = 2.5
bitperiod = 1.0/2e6
#~ # skip c-ca 9bits @ 2MBPs - 0.5e-6*9 = 4.5e-06
skip9ind = int((9.0*bitperiod)/dt)
# skip c-ca 10bits @ 2MBPs - 0.5e-6*9 = 4.5e-06
skip10ind = int((10.0*bitperiod)/dt)
skip95ind = int((9.5*bitperiod)/dt)
skip15ind = int((1.5*bitperiod)/dt)
skip25ind = int((2.5*bitperiod)/dt)
skip35ind = int((3.5*bitperiod)/dt)
skip45ind = int((4.5*bitperiod)/dt)
skip55ind = int((5.5*bitperiod)/dt)
skip65ind = int((6.5*bitperiod)/dt)
skip75ind = int((7.5*bitperiod)/dt)
skip85ind = int((8.5*bitperiod)/dt)
skip85ind = int((8.5*bitperiod)/dt)
skip1ind = int((1.0*bitperiod)/dt)
# ##### END 'global' consts:

# analysis
abmT = AnalysisByteMarks(ax_sub1, t, chan1)
abmB = AnalysisByteMarks(ax_sub2, t, chan2)
